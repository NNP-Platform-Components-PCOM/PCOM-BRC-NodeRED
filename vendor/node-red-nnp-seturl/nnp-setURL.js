...
module.exports = function (RED) {
    ...
    function NnpSetUrlNode(n) {
        RED.nodes.createNode(this, n);
        var node = this;
        this.url = n.url;
        this.context_path = n.context_path;
        this.rules = n.rules;
        var rule;
        if (!this.rules) {
            rule = {
        ...
        for (var i = 0; i < this.rules.length; i++) {
            ...
            if (rule.t === "change" && rule.fromt !== 'msg' && rule.fromt !== 'flow' && rule.fromt !== 'global') {
                ...
                try {
                    ...
                    valid = false;
                    this.error(RED._("nnpSetURL.errors.invalid-from", { error: e.message }));
                }
            }
            if (rule.tot === 'num') {
                ...
                try {
                    ...
                    valid = false;
                    this.error(RED._("nnpSetURL.errors.invalid-json"));
                }
                ...
                try {
                    ...
                    valid = false;
                    this.error(RED._("nnpSetURL.errors.invalid-expr", { error: e.message }));
                }
        ...
        function replaceParams(msg, patternStr, withStr) {
            //:\w+
            var regexp = new RegExp(":" + patternStr, "g");
            msg.url = msg.url.replace(regexp, withStr);
        }
        ...
        function getToValue(msg, rule, done) {
            ...
            if (rule.tot === "msg") {
                ...
                RED.util.evaluateJSONataExpression(rule.to, msg, (err, value) => {
                    if (err) {
                        done(RED._("nnpSetURL.errors.invalid-expr", { error: err.message }))
                    } else {
                        done(undefined, value);
                    }
                });
        ...
        function getFromValueType(fromValue, done) {
            ...
            if (typeof fromValue === 'number' || fromValue instanceof Number) {
                ...
                try {
                    ...
                    done(new Error(RED._("nnpSetURL.errors.invalid-from", { error: e.message })));
                }
            } else {
                done(new Error(RED._("nnpSetURL.errors.invalid-from", { error: "unsupported type: " + (typeof fromValue) })));
            }
            done(undefined, {
                fromType,
                fromValue,
                        ...
                        getFromValue(msg, rule, (err, fromParts) => {
                            if (err) {
                                ...
                                if (rule.pt === 'msg') {
                                    try {
                                        if (rule.t === 'delete') {
                                            ...
                                            if (!RED.util.setMessageProperty(msg, property, value)) {
                                                node.warn(RED._("nnpSetURL.errors.no-override", { property: property }));
                                            }
                                        } else if (rule.t === 'change') {
                                    ...
                                    replaceParams(msg, rule.p, encodeURIComponent(value));
                                    done(undefined, msg);
                                }
                            }
                        })
        ...
        if (valid) {
            this.on('input', function (msg, send, done) {
                msg.url = node.url + node.context_path;
    ...
    RED.httpAdmin.get("/settings/nnp-setURL/api", (req, res) => {
        if (RED.settings.nnpConfig.config['nnp-setURL']) {
            fetchApis(res);
        } else {
            http.get(RED.settings.nnpConfig.url, function (data, response) {
                ...
                RED.settings.nnpConfig.config['nnp-setURL'];
                fetchApis(res);
            });
        }
    });

    function fetchApis(res) {
        // console.log("calling ");
        const url = RED.settings.nnpConfig.config['nnp-setURL']['api-ecosystem-url'];
        const req = http.get(url, function (data, response) {
            // console.log("calling " + data);
            let result = [];
            if (response.statusCode === 200) {
                result = data;
            } else {
                RED.log.error(`Unable to fetch registered APIs from ${url} :::: Error = ${response.statusMessage}`);
            }
            result.push({
                    "apiid": "-999",
        ...
        req.on('error', function (err) {
            RED.log.error(`Unable to fetch registered APIs from ${url} :::: Error = `, err);
            res.json([]);
        });
    }


    RED.nodes.registerType("nnp-setURL", NnpSetUrlNode);
};
